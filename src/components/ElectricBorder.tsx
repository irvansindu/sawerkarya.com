...
type Props = {
  ...
  color?: string;
  speed?: number; // multiplier, >1 faster
  chaos?: number; // multiplier for displacement scale
  thickness?: number; // px
  className?: string;
...
export default function ElectricBorder({
  ...
  const updateAnim = () => {
    ...
    requestAnimationFrame(() => {
      [...dyAnims, ...dxAnims].forEach((a) => {
        // SVGAnimationElement should have beginElement in lib.dom.d.ts
        try {
  ...
  const vars: React.CSSProperties & { ["--electric-border-color"]?: string; ["--eb-border-width"]?: string } = {
